...
if TYPE_CHECKING:
    from src.iracing_manager import IRacingManagerInterface
    from src.app_settings import AppSettings
from src.utils import dict_to_xml_string # Import the new utility
...
class ReplayAnalyzer:
    ...
    def analyse_race(self, abort_check: Callable[[], bool]) -> Optional[str]:
        ...
        IRSDK_SESSION_STATE_RACING = 5 # from irsdk enums (conceptually)
...
if __name__ == '__main__':
    ...
    # Using IrsdkManager, which has its own internal mock if irsdk is not available.
    from src.irsdk_manager import IrsdkManager, IRSDK_AVAILABLE, IRSDK_TRK_LOC_ON_TRACK, IRSDK_TRK_LOC_ON_PIT_ROAD, IRSDK_TRK_LOC_IN_PIT_STALL
    if not IRSDK_AVAILABLE:
        pass

    mock_settings = MockAppSettings()
    pyir_manager = IrsdkManager() # Will use MockIRSDK if real not found

    # Configure the mock IrsdkManager for the demo to simulate a more complete race scenario
